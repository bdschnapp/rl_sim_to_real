#!/usr/bin/env bash
# Forward-only v7 post-train.
set -uo pipefail

FWD_PID=2514985
REPO=/home/ben/Ben/Thesis/Electrans_project
E2E_PY=/home/ben/Ben/Thesis/e2e_rl/venv/bin/python3
REEXPORT=$REPO/src/electrans_rl_bridge/scripts/re_export_td3.py
V_FWD_ZIP=$REPO/lab_models_v7/models/forward/lidar_24/multiplicative/best_model.zip
V_FWD_PTH=$REPO/lab_models_v7/models/forward/lidar_24/multiplicative/best_model.policy.pth
PREV_FWD_PTH=$REPO/lab_models_v3/models/forward/lidar_24/multiplicative/best_model.policy.pth

LOG=$REPO/lab_models_v7/.train_logs/post_train.log
exec > "$LOG" 2>&1

echo "[v7 post_train] waiting for PID $FWD_PID to exit"
while kill -0 "$FWD_PID" 2>/dev/null; do
  sleep 60
done
echo "[v7 post_train] forward PID exited at $(date)"

echo "[v7 post_train] forward final evaluations.npz:"
"$E2E_PY" - <<'PY'
import numpy as np
d = np.load('/home/ben/Ben/Thesis/Electrans_project/lab_models_v7/models/forward/lidar_24/multiplicative/logs/evaluations.npz')
r = d['results'].mean(axis=1)
ts = d['timesteps']
print(f'n_evals={len(ts)}, best={r.max():.1f} @ ts={ts[r.argmax()]}, final={r[-1]:.1f}')
for t, rv in zip(ts[-8:], r[-8:]):
    print(f'  ts={t:>7}  reward={rv:>8.1f}')
PY

echo
echo "[v7 post_train] re-exporting forward"
"$E2E_PY" "$REEXPORT" "$V_FWD_ZIP" "$V_FWD_PTH" 2>&1 | tail -4

echo
echo "[v7 post_train] repointing forward launches: v3 -> v7"
LAUNCHES=(
  "$REPO/src/launcher/autoware_launch/autoware_launch/launch/planning_simulator.launch.xml"
  "$REPO/src/launcher/autoware_launch/autoware_launch/launch/autoware.launch.xml"
  "$REPO/src/electrans_rl_bridge/launch/electrans_rl_bridge.launch.xml"
  "$REPO/src/launcher/autoware_launch/autoware_launch/launch/components/tier4_control_component.launch.xml"
)
for f in "${LAUNCHES[@]}"; do
  sed -i "s|$PREV_FWD_PTH|$V_FWD_PTH|g" "$f"
done

echo "[v7 post_train] rebuilding affected ROS packages"
cd "$REPO"
colcon build --symlink-install --packages-select electrans_rl_bridge autoware_launch 2>&1 | tail -4

echo
echo "[v7 post_train] done at $(date)"
echo "[v7 post_train] NOTE: reverse launches still point at v3 fixed-speed."
