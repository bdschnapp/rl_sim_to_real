...
V_FWD_ZIP=$REPO/lab_models_v13/models/forward/lidar_24/multiplicative/best_model.zip
V_FWD_PTH=$REPO/lab_models_v13/models/forward/lidar_24/multiplicative/best_model.policy.pth
PREV_FWD_PTH=$REPO/lab_models_v12/models/forward/lidar_24/multiplicative/best_model.policy.pth

LOG=$REPO/lab_models_v13/.train_logs/post_train.log
exec > "$LOG" 2>&1

echo "[v13 post_train] waiting for PID $FWD_PID"
while kill -0 "$FWD_PID" 2>/dev/null; do sleep 60; done
echo "[v13 post_train] training exited at $(date)"

"$E2E_PY" - <<'PY'
import numpy as np
d = np.load('/home/ben/Ben/Thesis/Electrans_project/lab_models_v13/models/forward/lidar_24/multiplicative/logs/evaluations.npz')
...
echo "[v13 post_train] re-exporting forward"
...
sed -i 's|lab_models_v12|lab_models_v13|g' "$REPO/src/electrans_rl_bridge/scripts/eval_lab_model.py"

cd "$REPO"
colcon build --symlink-install --packages-select electrans_rl_bridge autoware_launch 2>&1 | tail -4
echo "[v13 post_train] done at $(date)"
