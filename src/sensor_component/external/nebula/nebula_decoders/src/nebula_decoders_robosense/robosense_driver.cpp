...
#include "nebula_decoders/nebula_decoders_robosense/decoders/airy.hpp"
  ...
  const std::shared_ptr<const RobosenseCalibrationConfiguration> & calibration_configuration)
{
  ...
  switch (sensor_configuration->sensor_model) {
    ...
    case SensorModel::ROBOSENSE_BPEARL_V4:
      ...
      break;
    case SensorModel::ROBOSENSE_AIRY:
      scan_decoder_.reset(
        new RobosenseDecoder<Airy>(sensor_configuration, calibration_configuration));
      break;
