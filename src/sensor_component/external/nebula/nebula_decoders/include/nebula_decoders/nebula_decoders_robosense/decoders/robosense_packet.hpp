...
struct has_range_resolution : std::false_type
{
};

template <typename PacketT>
struct has_range_resolution<
  PacketT, std::void_t<decltype(std::declval<const PacketT &>().header.range_resolution.value())>>
: std::true_type
{
};
}  // namespace detail
...
{
  if constexpr (detail::has_range_resolution<PacketT>::value) {
  ...
  // Fallback for packet types that do not surface range resolution in the header
  return 0.0050;
}
