...
public:
  ...
  /// @brief Map a block-local channel index to the global channel id for calibration lookup
  /// @param block_id The current block id being processed
  /// @param channel_id The block-local channel id
  /// @return The global channel id
  [[nodiscard]] virtual uint32_t get_global_channel_id(
    uint32_t block_id, uint32_t channel_id) const
  {
    (void)block_id;
    return channel_id;
  }
    ...
    for (uint32_t block_id = start_block_id; block_id < start_block_id + n_returns; ++block_id) {
      for (uint32_t channel_id = 0; channel_id < PacketT::n_channels; ++channel_id) {
        min_offset_ns = std::min(
          min_offset_ns,
          get_packet_relative_point_time_offset(
            block_id, get_global_channel_id(block_id, channel_id), sensor_configuration));
      }
    }
