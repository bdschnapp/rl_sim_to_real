...
protected:
  ...
  void convert_returns(size_t start_block_id, size_t n_blocks)
  {
    ...
    for (size_t channel_id = 0; channel_id < SensorT::packet_t::n_channels; ++channel_id) {
      ...
      for (size_t block_offset = 0; block_offset < n_blocks; ++block_offset) {
        const size_t block_index = block_offset + start_block_id;
        const auto global_channel_id = sensor_.get_global_channel_id(block_index, channel_id);
        ...
        point.time_stamp =
          get_point_time_relative(packet_timestamp_ns, block_index, global_channel_id);

        point.return_type = static_cast<uint8_t>(return_type);

        auto corrected_angle_data =
          angle_corrector_.get_corrected_angle_data(raw_azimuth, global_channel_id);
