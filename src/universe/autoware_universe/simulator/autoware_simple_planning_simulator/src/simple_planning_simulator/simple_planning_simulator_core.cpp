...
: Node("simple_planning_simulator", options), tf_buffer_(get_clock()), tf_listener_(tf_buffer_)
{
  ...
  pub_trailer_state_ = create_publisher<TrailerState>("output/trailer_state", QoS{1});
...
void SimplePlanningSimulator::initialize_vehicle_model(const std::string & vehicle_model_type_str)
{
  ...
  if (vehicle_model_type_str == "IDEAL_STEER_VEL") {
    ...
    vehicle_model_ptr_ = std::make_shared<SimModelDelaySteerMapAccGeared>(
      ...
      acceleration_map_path);
  } else if (vehicle_model_type_str == "DELAY_STEER_ACC_GEARED_TRAILER") {
    vehicle_model_type_ = VehicleModelType::DELAY_STEER_ACC_GEARED_TRAILER;
    trailer_hitch_offset_ = declare_parameter("hitch_offset", -0.5);
    trailer_wheelbase_ = declare_parameter("trailer_wheelbase", 6.0);
    const double max_hitch_angle = declare_parameter("max_hitch_angle", 0.785);
    vehicle_model_ptr_ = std::make_shared<SimModelDelaySteerAccGearedTrailer>(
      vel_lim, steer_lim, vel_rate_lim, steer_rate_lim, wheelbase,
      trailer_hitch_offset_, trailer_wheelbase_, max_hitch_angle,
      timer_sampling_time_ms_ / 1000.0,
      acc_time_delay, acc_time_constant, steer_time_delay, steer_time_constant,
      steer_dead_band, steer_bias, debug_acc_scaling_factor, debug_steer_scaling_factor);
...
void SimplePlanningSimulator::on_timer()
{
  ...
  if (vehicle_model_type_ == VehicleModelType::DELAY_STEER_ACC_GEARED_TRAILER) {
    publish_trailer_state();
  }
}
...
void SimplePlanningSimulator::set_initial_state(const Pose & pose, const Twist & twist)
{
  ...
  } else if (vehicle_model_type_ == VehicleModelType::DELAY_STEER_ACC_GEARED_TRAILER) {
    // 7th state: hitch_angle initialized to 0 (truck and trailer aligned)
    state << x, y, yaw, vx, steer, accx, 0.0;
  }
...
void SimplePlanningSimulator::publish_trailer_state()
{
  const double truck_yaw = vehicle_model_ptr_->getYaw();
  ...
  const double truck_x = current_odometry_.pose.pose.position.x;
  const double truck_y = current_odometry_.pose.pose.position.y;
  const double truck_z = current_odometry_.pose.pose.position.z;

  // hitch point = truck rear-axle + hitch_offset along truck heading
  const double hitch_x = truck_x + trailer_hitch_offset_ * std::cos(truck_yaw);
  const double hitch_y = truck_y + trailer_hitch_offset_ * std::sin(truck_yaw);

  // trailer rear axle = hitch - trailer_wheelbase along trailer heading
  const double trailer_x = hitch_x - trailer_wheelbase_ * std::cos(trailer_yaw);
  const double trailer_y = hitch_y - trailer_wheelbase_ * std::sin(trailer_yaw);

  // Build quaternion from trailer yaw
  tf2::Quaternion q;
  q.setRPY(0.0, 0.0, trailer_yaw);

  // Publish trailer_link TF (same /tf topic used for base_link)
  TransformStamped tf;
  tf.header.stamp = get_clock()->now();
  tf.header.frame_id = origin_frame_id_;
  tf.child_frame_id = "trailer_link";
  tf.transform.translation.x = trailer_x;
  tf.transform.translation.y = trailer_y;
  tf.transform.translation.z = truck_z;
