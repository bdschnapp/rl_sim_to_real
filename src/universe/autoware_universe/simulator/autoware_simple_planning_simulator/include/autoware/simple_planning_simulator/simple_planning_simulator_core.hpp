...
using autoware_vehicle_msgs::msg::TrailerState;
...
private:
  ...
  rclcpp::Publisher<TrailerState>::SharedPtr pub_trailer_state_;
  ...
  enum class VehicleModelType {
    ...
    DELAY_STEER_ACC_GEARED_TRAILER = 13,
  ...
  // Trailer geometry — set by initialize_vehicle_model() for DELAY_STEER_ACC_GEARED_TRAILER
  ...
  /**
   * @brief publish trailer hitch state and trailer_link TF
   * @details Only called when vehicle_model_type_ == DELAY_STEER_ACC_GEARED_TRAILER.
   */
  void publish_trailer_state();
