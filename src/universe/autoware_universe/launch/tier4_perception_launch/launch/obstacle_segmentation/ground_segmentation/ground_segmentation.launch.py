...
class GroundSegmentationPipeline:
    ...
    def create_single_frame_outlier_filter_components(input_topic, output_topic, context):
        # Single-frame outlier filter pipeline used autoware_elevation_map_loader +
        # autoware_compare_map_segmentation, both removed during the aggressive prune.
        # Returning an empty component list disables this branch. The pipeline still
        # has the time-series outlier filter available (autoware_occupancy_grid_map_outlier_filter
        # is kept), so a working perception config should select that variant.
        return []
